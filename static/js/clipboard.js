...
function unwrap(option, default$2) {
...
function get(map5, key2) {
  const value2 = map5?.get(key2);
  if (value2 != null) {
...
function has_key2(map5, key2) {
  return map5 && map5.has(key2);
}
function insert2(map5, key2, value2) {
  map5 ??= /* @__PURE__ */ new Map();
  map5.set(key2, value2);
  return map5;
}
function remove(map5, key2) {
  map5?.delete(key2);
  return map5;
}
...
function add2(parent, index3, key2) {
  if (key2 === "") {
...
var addEventListener = (node, name, handler, options2) => node.addEventListener(name, handler, options2);
...
var insertMetadataChild = (kind, parent, node, index3, key2) => {
...
var Reconciler = class {
  ...
  #removeDebouncers(node) {
    ...
    for (const { timeout: timeout2 } of debouncers.values()) {
      if (timeout2) {
  ...
  #createElement(parent, index3, { kind, key: key2, tag, namespace, attributes }) {
    const node = createElementNS(namespace || NAMESPACE_HTML, tag);
    insertMetadataChild(kind, parent, node, index3, key2);
  ...
  #createTextNode(parent, index3, { kind, key: key2, content }) {
  ...
  #updateDebounceThrottle(map5, name, delay) {
    ...
    if (delay > 0) {
      ...
      const { timeout: timeout2 } = debounceOrThrottle;
      if (timeout2) {
        clearTimeout(timeout2);
      }
      map5.delete(name);
...
var virtualiseNode = (meta2, node, key2, index3) => {
  if (!canVirtualiseNode(node)) {
    return null;
  }
  switch (node.nodeType) {
    case ELEMENT_NODE: {
      const childMeta = insertMetadataChild(element_kind, meta2, node, index3, key2);
...
var virtualiseChildNodes = (meta2, node) => {
  ...
  while (child) {
    ...
    const vnode = virtualiseNode(meta2, child, key2, index3);
...
var Runtime = class {
  constructor(root3, [model, effects], view3, update3) {
    ...
    this.root.addEventListener("context-request", (event4) => {
      ...
      const context2 = this.#contexts.get(event4.context);
      if (event4.subscribe) {
        const callbackRef = new WeakRef(event4.callback);
        const unsubscribe = () => {
          context2.subscribers = context2.subscribers.filter(
            (subscriber) => subscriber !== callbackRef
          );
        };
        context2.subscribers.push([callbackRef, unsubscribe]);
        event4.callback(context2.value, unsubscribe);
      } else {
        event4.callback(context2.value);
      }
    });
  ...
  provide(key2, value2) {
    if (!this.#contexts.has(key2)) {
      ...
      const context2 = this.#contexts.get(key2);
      context2.value = value2;
      for (let i = context2.subscribers.length - 1; i >= 0; i--) {
        const [subscriberRef, unsubscribe] = context2.subscribers[i];
        const subscriber = subscriberRef.deref();
        if (!subscriber) {
          context2.subscribers.splice(i, 1);
          continue;
        }
...
var Spa = class {
  #runtime;
  constructor(root3, [init, effects], update3, view3) {
    this.#runtime = new Runtime(root3, [init, effects], view3, update3);
  }
  send(message) {
...
var start = ({ init, update: update3, view: view3 }, selector, flags) => {
  if (!is_browser())
    return new Error2(new NotABrowser());
  const root3 = selector instanceof HTMLElement ? selector : document2().querySelector(selector);
  if (!root3)
    return new Error2(new ElementNotFound(selector));
  return new Ok(new Spa(root3, init(flags), update3, view3));
};

// build/dev/javascript/lustre/lustre.mjs
var App = class extends CustomType {
  constructor(init, update3, view3, config) {
    super();
    this.init = init;
    this.update = update3;
    this.view = view3;
...
function application(init, update3, view3) {
  return new App(init, update3, view3, new$7(empty_list));
}
...
// build/dev/javascript/grille_pain/grille_pain/error.mjs
...
var ContextError = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function lustre(res) {
  return map_error(res, (var0) => {
    return new LustreError(var0);
  });
}
function context(res, context2) {
  return replace_error(res, new ContextError(context2));
}

// build/dev/javascript/grille_pain/grille_pain/internals/unsafe.ffi.mjs
function coerce(value2) {
  return value2;
}

// build/dev/javascript/grille_pain/grille_pain/internals/global.ffi.mjs
function setTimeout2(timeout2, fn) {
  window.setTimeout(fn, timeout2);
}
...
// build/dev/javascript/grille_pain/grille_pain/internals/element.ffi.mjs
function create(tag) {
  if (typeof document === "undefined")
    return new Error2();
  return new Ok(document.createElement(tag));
}
function attachShadow(element4) {
  if (element4.attachShadow === void 0)
    return new Error2();
  return new Ok(element4.attachShadow({ mode: "open" }));
}
function appendChild(root3, child) {
  root3.appendChild(child);
  return new Ok(root3);
}
function body() {
  if (typeof document === "undefined")
    return new Error2();
  return new Ok(document.body);
}
...
// build/dev/javascript/grille_pain/grille_pain/toast/level.mjs
var Standard = class extends CustomType {
};
var Info = class extends CustomType {
};
var Warning = class extends CustomType {
};
var Error3 = class extends CustomType {
};
function to_string4(level2) {
  if (level2 instanceof Standard) {
    return "Standard";
  } else if (level2 instanceof Info) {
    return "Info";
  } else if (level2 instanceof Warning) {
    return "Warning";
  } else if (level2 instanceof Error3) {
    return "Error";
  } else {
    return "Success";
  }
...
// build/dev/javascript/grille_pain/grille_pain/internals/data/toast.ffi.mjs
function computeBottomPosition(root3, id) {
  const [...nodes] = root3.querySelectorAll(".grille_pain-toast");
  const result = nodes.reduce((acc, node) => {
    if (node.classList.contains("grille_pain-toast-will-hide"))
      return acc;
...
// build/dev/javascript/grille_pain/grille_pain/internals/data/toast.mjs
var WillShow = class extends CustomType {
};
var Show = class extends CustomType {
};
var WillHide = class extends CustomType {
};
var Toast = class extends CustomType {
  constructor(external_id, id, sticky, message, displayed, running, remaining, last_schedule, iteration, bottom, level2, animation_duration) {
    super();
...
function running_to_string(running) {
  if (running) {
    return "running";
  } else {
    return "paused";
...
function by_uuid(toast, uuid2) {
  return toast.external_id === uuid2;
}
function by_id(toast, id) {
  return toast.id === id;
}
function by_iteration(toast, id, iteration) {
  return toast.id === id && toast.iteration === iteration;
}
function new$8(external_id, id, message, level2, remaining, sticky, root3) {
  return new Toast(
    external_id,
    id,
    sticky,
    ...
    now(),
    0,
    computeBottomPosition(root3, id),
    level2,
    remaining
  );
}

// build/dev/javascript/grille_pain/grille_pain/internals/data/model.mjs
var Model = class extends CustomType {
  constructor(toasts, id, timeout2, root3, next_frame, to_show) {
    super();
    this.toasts = toasts;
    this.id = id;
...
var ToShow = class extends CustomType {
  constructor(toast_id, timeout2, sticky) {
    super();
...
function new$9(root3, timeout2) {
  let toasts = toList([]);
  let id = 0;
  return new Model(toasts, id, timeout2, root3, new None(), toList([]));
}
function add3(model, external_id, message, level2, sticky, timeout2) {
  let animation_duration = unwrap(timeout2, model.timeout);
  let new_toast = new$8(
    external_id,
    model.id,
    message,
  ...
  let new_toasts = prepend(new_toast, model.toasts);
  let new_id = model.id + 1;
  let to_show = prepend(
    new ToShow(new_toast.id, timeout2, sticky),
    model.to_show
  );
  return new Model(
    new_toasts,
...
function stop(model, id) {
  return update_toast(
    model,
    id,
    (toast) => {
      let now2 = now();
      let duration = now2 - toast.last_schedule;
...
function remove3(model, id) {
  let new_toasts = filter(
    model.toasts,
    (toast) => {
      return toast.id !== id;
    }
...
function update_bottom_positions(model) {
  return new Model(
    map2(
      model.toasts,
      (toast) => {
        let $ = toast.displayed;
        if ($ instanceof WillShow) {
          return new Toast(
            toast.external_id,
            toast.id,
            toast.sticky,
            ...
            toast.last_schedule,
            toast.iteration,
            computeBottomPosition(model.root, toast.id),
            toast.level,
            toast.animation_duration
          );
        } else if ($ instanceof Show) {
          return new Toast(
            toast.external_id,
            toast.id,
            toast.sticky,
...
// build/dev/javascript/grille_pain/grille_pain/internals/data/msg.mjs
var BrowserUpdatedToasts = class extends CustomType {
};
var LustreComputedToasts = class extends CustomType {
};
var LustreRequestedAnimationFrame = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var ToastHidDisplay = class extends CustomType {
  constructor(id) {
    super();
    this.id = id;
  }
};
var ToastTimedOut = class extends CustomType {
...
var UserAddedToast = class extends CustomType {
  constructor(uuid2, message, level2, timeout2, sticky) {
    super();
    this.uuid = uuid2;
    this.message = message;
    this.level = level2;
...
var UserEnteredToast = class extends CustomType {
  constructor(id) {
    super();
    this.id = id;
  }
};
var UserHidToast = class extends CustomType {
  constructor(uuid2) {
    super();
    this.uuid = uuid2;
  }
};
var UserLeavedToast = class extends CustomType {
  constructor(id) {
    super();
    this.id = id;
  }
...
// build/dev/javascript/grille_pain/grille_pain/internals/effect_manager.ffi.mjs
var storeDispatcher_ = null;
...
// build/dev/javascript/grille_pain/grille_pain/internals/lustre/schedule.mjs
function schedule(duration, msg) {
  return from(
    (dispatch2) => {
      return setTimeout2(duration, () => {
...
// build/dev/javascript/grille_pain/grille_pain/internals/setup.mjs
function do_mount() {
  return try$(
    create("grille-pain"),
    (node) => {
      return try$(
        create("div"),
        (lustre_root_) => {
          let lustre_root = coerce(lustre_root_);
          return try$(
            attachShadow(node),
            (shadow) => {
              return try$(
                appendChild(shadow, lustre_root_),
                (shadow2) => {
                  return try$(
                    addStyles(shadow2),
                    (shadow3) => {
                      return try$(
                        body(),
                        (body3) => {
                          let $ = appendChild(body3, node);
                          return new Ok([lustre_root, shadow3]);
                        }
                      );
                    }
                  );
...
function mount2() {
  let _pipe = do_mount();
  return context(_pipe, "Impossible to mount grille_pain.");
}
...
// build/dev/javascript/grille_pain/grille_pain/internals/css.mjs
function var$(content, default$2) {
  return "var(--" + content + ", " + default$2 + ")";
}

// build/dev/javascript/grille_pain/grille_pain/internals/view/theme.ffi.mjs
function isDark() {
  const matches2 = matchMedia("(prefers-color-scheme: dark)");
  return matches2.matches;
}

// build/dev/javascript/grille_pain/grille_pain/internals/view/theme.mjs
var info = "var(--info)";
var success2 = "var(--success)";
var warning = "var(--warning)";
var error = "var(--error)";
var color_transparent = "var(--color-transparent)";
var light_transparent = "var(--light-transparent)";
var dark_transparent = "var(--dark-transparent)";
var dark = "var(--dark)";
var light = "var(--light)";
function standard() {
  let $ = isDark();
...
function color(level2) {
  if (level2 instanceof Standard) {
    ...
    return [success2, light];
  }
}

// build/dev/javascript/grille_pain/grille_pain/internals/view/progress_bar.mjs
function background_color(level2) {
  if (level2 instanceof Standard) {
    let $ = isDark();
    if ($) {
...
function pb_background_color(level2) {
  let back_color = background_color(level2);
  let level$1 = to_string4(level2);
  let variable = "grille_pain-" + level$1 + "-progress-bar";
  return var$(variable, back_color);
}
function view(toast) {
  let animation_duration = to_string(toast.animation_duration);
  let play_state = running_to_string(toast.running);
  return div(
    toList([
      class$("progress-bar"),
      style("animation-duration", animation_duration + "ms"),
      style("background", pb_background_color(toast.level)),
...
// build/dev/javascript/grille_pain/grille_pain/internals/view.mjs
function select_on_click_action(toast) {
  return lazy_guard(
    toast.sticky,
    none,
    () => {
      return on_click(new ToastTimedOut(toast.id, toast.iteration));
    }
  );
}
...
function toast_wrapper(toast, attributes, children) {
  return div(
    prepend(
      class$("toast-wrapper"),
      prepend(
        class$(toast_wrapper_class(toast)),
        prepend(
          style("right", toast_wrapper_right(toast)),
          prepend(
            style("top", to_string(toast.bottom) + "px"),
            attributes
          )
        )
      )
...
function wrapper_dom_classes(toast) {
  ...
  return classes(
    toList([
      ...
      ["grille_pain-toast-" + to_string(toast.id), true],
      ["grille_pain-toast-" + displayed, true]
    ])
  );
}
function toast_colors(level2) {
  let $ = color(level2);
  let background;
  let text_color;
  background = $[0];
  ...
  let text4 = "grille_pain-" + level$1 + "-text-color";
  let bg = style("background", var$(background_, background));
  let color2 = style("color", var$(text4, text_color));
  return toList([bg, color2]);
}
function toast_container(toast, children) {
  let mouse_enter = on_mouse_enter(new UserEnteredToast(toast.id));
  let mouse_leave = on_mouse_leave(new UserLeavedToast(toast.id));
  let colors = toast_colors(toast.level);
  let toast$1 = class$("toast");
  return div(
    prepend(
      mouse_enter,
      prepend(mouse_leave, prepend(toast$1, colors))
    ),
    children
  );
}
function toast_content(attributes, children) {
  return div(
    prepend(class$("toast-content"), attributes),
    children
  );
}
function view_toast(toast) {
  let on_hide = select_on_click_action(toast);
  let data_id = attribute2("data-id", to_string(toast.id));
  return toast_wrapper(
    ...
    toList([
      toast_container(
        toast,
        toList([
          toast_content(toList([on_hide]), toList([text3(toast.message)])),
...
function view2(model) {
  ...
  return div2(
    toList([class$("grille-pain")]),
    map2(
      toasts,
      (toast) => {
        let id = to_string(toast.id);
...
// build/dev/javascript/grille_pain/grille_pain/options.mjs
...
function default$() {
  return new Options(5e3);
}
function timeout(_, timeout2) {
  return new Options(timeout2);
}

// build/dev/javascript/grille_pain/grille_pain.mjs
function schedule_hide(sticky, timeout2, id, iteration) {
  return lazy_guard(
    sticky,
    none2,
    () => {
      return schedule(timeout2, new ToastTimedOut(id, iteration));
    }
  );
}
function show2(model, id, timeout2, sticky) {
  let time = unwrap(timeout2, model.timeout);
  let new_model = show(model, id);
...
function update_display(model) {
  ...
  if ($ instanceof Some) {
    ...
    return from(
      (dispatch2) => {
        return dispatch2(
          new LustreRequestedAnimationFrame(
            requestAnimationFrame2(
              () => {
                return dispatch2(new BrowserUpdatedToasts());
              }
            )
          )
        );
      ...
      (dispatch2) => {
        return dispatch2(
          new LustreRequestedAnimationFrame(
            requestAnimationFrame2(
              () => {
                return dispatch2(new LustreComputedToasts());
              }
            )
          )
        );
...
function update2(model, msg) {
  if (msg instanceof BrowserUpdatedToasts) {
    ...
    let model$2 = update_bottom_positions(model$1);
    return [model$2, update_toasts(model$2)];
  } else if (msg instanceof LustreComputedToasts) {
    let model$1 = new Model(
      model.toasts,
      model.id,
      model.timeout,
      ...
      return ((_capture) => {
        return fold2(
          ...
          (model2, showable2) => {
            ...
            let timeout2;
            let sticky;
            toast_id = showable2.toast_id;
            timeout2 = showable2.timeout;
            sticky = showable2.sticky;
            let model$12;
            let effs;
            model$12 = model2[0];
            effs = model2[1];
            let $1 = show2(model$12, toast_id, timeout2, sticky);
            let model$22;
            let eff;
            model$22 = $1[0];
            eff = $1[1];
            return [model$22, batch(toList([effs, eff]))];
          }
        );
      })(_pipe);
    }
  } else if (msg instanceof LustreRequestedAnimationFrame) {
    let frame = msg[0];
    let next_frame = new Some(frame);
    let model$1 = new Model(
      model.toasts,
      model.id,
      model.timeout,
    ...
    return [model$1, none2()];
  } else if (msg instanceof ToastHidDisplay) {
    let id = msg.id;
    let model$1 = remove3(model, id);
    let update$1 = update_display(model$1);
    return [model$1, update$1];
  } else if (msg instanceof ToastTimedOut) {
    let id = msg.id;
    let iteration = msg.iteration;
    let $ = find(
      model.toasts,
      (_capture) => {
        return by_iteration(_capture, id, iteration);
      }
    );
    if ($ instanceof Ok) {
      let toast = $[0];
      let model$1 = hide(model, toast.id);
      let to_remove = schedule(1e3, new ToastHidDisplay(id));
      let update$1 = update_display(model$1);
      return [model$1, batch(toList([to_remove, update$1]))];
    } else {
      return [model, none2()];
    }
  } else if (msg instanceof UserAddedToast) {
    let uuid2 = msg.uuid;
    let message = msg.message;
    let level2 = msg.level;
    let timeout2 = msg.timeout;
    let sticky = msg.sticky;
    let _pipe = add3(model, uuid2, message, level2, sticky, timeout2);
    return new$(_pipe, update_display(model));
  } else if (msg instanceof UserEnteredToast) {
    let id = msg.id;
    let model$1 = stop(model, id);
    return [model$1, none2()];
  } else if (msg instanceof UserHidToast) {
    let uuid2 = msg.uuid;
    let $ = find(
      model.toasts,
      (_capture) => {
        return by_uuid(_capture, uuid2);
      }
    );
    if ($ instanceof Ok) {
      let toast = $[0];
      return [
        ...
        from(
          (dispatch2) => {
            return dispatch2(new ToastTimedOut(toast.id, toast.iteration));
          }
        )
      ];
    } else {
  ...
  return try$(
    ...
    (_use0) => {
      ...
      return map4(
        (() => {
          let _pipe = (_) => {
            return [new$9(shadow, opts.timeout), none2()];
          };
          let _pipe$1 = application(_pipe, update2, view2);
          let _pipe$2 = start3(_pipe$1, lustre_root, void 0);
          return lustre(_pipe$2);
        })(),
        (dispatcher) => {
...
// build/dev/javascript/grille_pain/grille_pain/toast.mjs
var Options2 = class extends CustomType {
  constructor(timeout2, level2, sticky) {
    super();
    this.timeout = timeout2;
    this.level = level2;
    this.sticky = sticky;
...
function options() {
  return new Options2(new None(), new None(), false);
}
function level(options2, level2) {
  return new Options2(options2.timeout, new Some(level2), options2.sticky);
}
function dispatch_toast(options2, message) {
  return tap(
    uuid(),
    (uuid2) => {
      return call(
        (runtime) => {
          ...
          let level$1;
          let sticky$1;
          timeout$1 = options2.timeout;
          level$1 = options2.level;
          sticky$1 = options2.sticky;
          let level$2 = unwrap(level$1, new Standard());
          let _pipe = new UserAddedToast(
            uuid2,
            message,
            level$2,
            timeout$1,
...
function info2(content) {
  let _pipe = options();
  let _pipe$1 = level(_pipe, new Info());
...
async function writeText(clipText) {
  try {
    ...
    return new Error2(error2.toString());
  }
}
...
function getAttribute2(element4, name) {
  let attribute3 = element4.getAttribute(name);
  if (attribute3) {
    return new Ok(attribute3);
  }
  return new Error2();
}
function addEventListener2(element4, type, listener) {
  return element4.addEventListener(type, listener);
}

// build/dev/javascript/website/components/clipboard.mjs
var FILEPATH = "src/components/clipboard.gleam";
function main() {
  let _block;
  let _pipe = querySelectorAll("[copy_button]");
  _block = to_list(_pipe);
  let elements = _block;
  let _block$1;
  let _pipe$1 = default$();
  let _pipe$2 = timeout(_pipe$1, 2e3);
  _block$1 = setup(_pipe$2);
  let $ = _block$1;
  if (!($ instanceof Ok)) {
    throw makeError(
      "let_assert",
      ...
      { value: $, start: 341, end: 429, pattern_start: 352, pattern_end: 357 }
    );
  }
  return index_map(
    elements,
    (element4, _) => {
      let _block$2;
      let _pipe$3 = getAttribute2(element4, "data-copy");
      _block$2 = unwrap2(_pipe$3, "");
      let copy_text = _block$2;
